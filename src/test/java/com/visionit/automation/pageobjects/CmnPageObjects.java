...
//	private By tshirt_link=By.xpath("//*[@id=\"block_top_menu\"]/ul/li[3]/a");
	...
	//----------Valiadte Title of page------------
	public void validatePageTitle(String titleOfPage)
	{
		String actPageTitle= driver.getTitle();
    	Assert.assertEquals(titleOfPage, actPageTitle);
        logger.info("Validate title of page, title is:" + actPageTitle);
	}
	
	//--------------Validate Logo Display---------
	public void validateLogoDisplay()
	{
		WebElement logo =driver.findElement(nav_link_logo);
   ...
   //------------------1 Logo validation width --------------------
	public void validateLogowidth(int width) {
		...
		logger.info("logo validated with width : "+width);
	}
	
	//------------------1 Logo validation Height --------------------
	
	public void validateLogoheight(int height)
	{
		boolean c = driver.findElement(nav_link_logo).isDisplayed();
		Assert.assertEquals("Navigation link logo",true, c);

		int height1=driver.findElement(nav_link_logo).getSize().getHeight();

		Assert.assertEquals(height,height1);
		logger.info("logo validated with height : "+height);
	}
